...
require("../model/helpermethod.php");
...
if (isset($_POST["submit"])) {
  ...
  $login_result = fetch_row("registration_tb", array("email_address" => $var_email, "password" => $var_password,"role"=>"1"));
  $rows = mysqli_fetch_assoc($login_result);

  if (mysqli_num_rows($login_result) > 0) {
    if ($rows["email_address"] == $var_email && $rows["password"] == $var_password) {
      $_SESSION["id"] = $rows['id'];
      $_SESSION["admin"] = $rows["email_address"];
      $_SESSION["name"] = $rows["u_f_name"];
      if($rows["role"]=="1"){
				redirect("home.php");
			}
      else{
